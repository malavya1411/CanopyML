...
class Settings(BaseSettings):
    model_config = SettingsConfigDict(
        env_file=_root / "apps" / "backend" / ".env", env_file_encoding="utf-8", extra="ignore"
    )
    ...
    saved_models_dir: Path = _root / "models" / "saved_models"
    reports_dir:      Path = _root / "reports"
    eval_dir:         Path = _root / "data" / "logs" / "evaluation"
    ...
    def model_post_init(self, __context) -> None:
        ...
        for d in [self.saved_models_dir, self.reports_dir,
                  self.eval_dir, self.upload_dir, self.logs_dir]:
